...
PyObject* PythonTagGroup_mp_subscript(PyObject *self, PyObject *key)
{
  ...
  char buffer[64];
	...
	{
		sprintf(buffer, "[%d]", PyInt_AsLong(key));
		skey=buffer;
	}
  else { 
    SWIG_exception_fail(SWIG_ValueError, "in method 'TagGroup::__subscript__', argument 2 of type 'String' or 'int'"); 
  } 
  Gatan::DM::TagGroup *tg = reinterpret_cast<Gatan::DM::TagGroup*>(argp);
...
//some applys for outputting values.
//used for ApplicationGetBounds
%apply long *OUTPUT  {long *t, long *l, long *b, long *r};
//ApplicationGetOrigin
%apply long *OUTPUT  {long *x, long *y};
//font attributes
%apply long *OUTPUT  {long *attributes};
%apply unsigned long *OUTPUT  {ulong *attributes, ulong *size};
//FM_PrepareDMA
%apply long *OUTPUT  {long *logicalAddress, long *physicalAddress};
//image dimensions
%apply long *OUTPUT  {long *d0};
%apply long *OUTPUT  {long *d1};
%apply long *OUTPUT  {long *d2};
//application info
%apply long *OUTPUT  {long *info};
//error info
%apply short *OUTPUT  {short *error};
%apply long *OUTPUT  {long *context};
//GetInteger, also maybe even some others?
%apply long *OUTPUT  {long *result};
//stream info
%apply long *OUTPUT  {long *pos};
%apply long *OUTPUT  {long *size};
%apply long *OUTPUT  {long *width, long *height};
%apply long *OUTPUT  {long *xpos, long *ypos};
%apply long *OUTPUT  {long *start, long *end};
%apply long *OUTPUT  {long *top, long *left};
%apply long *OUTPUT  {long *bottom, long *right};
%apply long *OUTPUT  {long *power};
%apply long *OUTPUT  {long *number};
%apply unsigned long *OUTPUT  {ulong *number};
//and some bools
%apply bool *OUTPUT  {bool *result};
%apply bool *OUTPUT  {bool *doAutoSurveyLow, bool *doAutoSurveyHigh};
%apply bool *OUTPUT  {bool *val};
%apply bool *OUTPUT  {bool *fresh};
//floats
%apply float *OUTPUT  {float *result};
%apply float *OUTPUT  {float *number};
%apply float *OUTPUT  {float *t, float *l, float *b, float *r};
%apply float *OUTPUT  {float *top, float *left, float *bottom, float *right};
%apply float *OUTPUT  {float *off_x, float *off_y};
%apply float *OUTPUT  {float *scale_x, float *scale_y};
%apply float *OUTPUT  {float *x, float *y};
%apply float *OUTPUT  {float *x_window, float *y_window};
%apply float *OUTPUT  {float *horz, float *vert};
%apply float *OUTPUT  {float *xs, float *ys};
%apply float *OUTPUT  {float *minPtr, float *maxPtr};
%apply float *OUTPUT  {float *min, float *max};
%apply float *OUTPUT  {float *lowPtr, float *highPtr};
%apply float *OUTPUT  {float *low, float *high};
%apply float *OUTPUT  {float *bright, float *contrast};
%apply float *OUTPUT  {float *lowLimit, float *highLimit };
%apply float *OUTPUT  {float *leftChannel, float *rightChannel};
%apply float *OUTPUT  {float *origin, float *scale};
%apply float *OUTPUT  {float *r, float *g, float *b};
%apply float *OUTPUT  {float *sx, float *sy, float *ex, float *ey};
%apply float *OUTPUT  {float *start, float *end};
%apply float *OUTPUT  {float *x_axis_x, float *x_axis_y, float *y_axis_x, float *y_axis_y, float *z_axis};
//doubles
%apply double *OUTPUT  {double *number};
%apply double *OUTPUT  {double *r, double *g, double *b };
%apply double *OUTPUT  {double *o_o_x, double *o_o_y, double *o_s_x, double *o_s_y};
%apply double *OUTPUT  {double *o_p_x, double *o_p_y};
%apply double *OUTPUT  {double *o_r_t, double *o_r_l, double *o_r_b, double *o_r_r};

//ok ,lets try a typemap for importing DM Strings
%typemap(in) const Gatan::DM::String& (Gatan::DM::String tempstring) {
    tempstring=PyString_AsString($input);
    $1 = &tempstring;
};

%typemap(out) Gatan::DM::String {
    $result = PyString_FromString( $1.get_string().c_str());
};
#define _GATANPLUGIN_USES_LIBRARY_VERSION 2
//
